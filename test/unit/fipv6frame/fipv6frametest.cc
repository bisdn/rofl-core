...
fipv6frameTest::setUp()
{
	...
	(*(mem_A))[0] = 0x6a; // version = 6, tc[higher nibble]=5
	(*(mem_A))[1] = 0xa3; // tc[lower nibble]=4, flow-label=3
...
{
	uint8_t tc = 0x77;
	ipv6_A->set_traffic_class(tc); // traffic class = 0x77

	printf("ipv6: %s\n", ipv6_A->c_str());

	CPPUNIT_ASSERT(((((*mem_A)[0] & 0b00001111) << 4) + (((*mem_A)[1] & 0b11110000) >> 4)) == tc);
	CPPUNIT_ASSERT(ipv6_A->get_traffic_class() == tc);
}
