...
#include "../../../../../../src/rofl/pipeline/openflow/openflow12/pipeline/of12_statistics.h"
...
typedef struct
{
	of12_pipeline_t* pipe;
	of12_flow_entry_t* entries;
	of12_stats_port_t *port_stats;
	of12_stats_queue_t *queue_stats;
	int core_number;
} core_args_t;
...
void * core_main (void * args)
{
	...
	{
		//flow
		of12_stats_flow_inc(&(info->entries[0]), num_bytes);
		of12_stats_flow_duration_update(&(info->entries[0]));
		of12_stats_flow_get_duration(&info->entries[0]);
		
		//table
		of12_stats_table_lookup_inc(info->pipe->tables);
		if(info->core_number%2) of12_stats_table_matches_inc(info->pipe->tables);
		
		//aggregate
		
		//port
		of12_stats_port_rx_packet_inc(info->port_stats, num_bytes);
		of12_stats_port_tx_packet_inc(info->port_stats, num_bytes);
		
		//queue
		of12_stats_queue_tx_packet_inc(info->queue_stats, num_bytes);
		of12_stats_queue_tx_errors_inc(info->queue_stats);
	}
	
	return NULL;
}

void setup(of12_pipeline_t * pipeline, of12_flow_table_t * table, of12_flow_entry_t ** entries, of12_stats_port_t *port_stats, of12_stats_queue_t *queue_stats)//, platform_mutex_t *mutex_time)
{
	pipeline->tables = table;
	(*entries) = malloc(sizeof(of12_flow_entry_t)*STATS_TEST_MAX_ENTRIES);
	if((*entries)==NULL)
	{
	...
	of12_time_forward(STATS_TEST_SEC, STATS_TEST_USEC, NULL);
	//platform_mutex_init(mutex_time, NULL);
	table->mutex = platform_mutex_init(NULL);
	of12_stats_flow_init(entries[0]);
	of12_stats_table_init(pipeline->tables);
	of12_stats_port_init(port_stats);
	of12_stats_queue_init(queue_stats);
	
}

void clean_up(of12_flow_table_t* table, of12_flow_entry_t ** entries, of12_stats_port_t *port_stats, of12_stats_queue_t *queue_stats)//, platform_mutex_t *mutex_time)
{
	of12_stats_flow_destroy(entries[0]);
	//platform_mutex_destroy(mutex_time);
	platform_mutex_destroy(table->mutex);
	of12_stats_port_destroy(port_stats);
	of12_stats_queue_destroy(queue_stats);
	free((*entries));
}
...
int main(int args, char** argv)
{
	...
	for(i=0;i<STATS_TEST_NUM_OF_CORES;i++)
	{
		...
		of12_time_forward(rand_sec,rand_usec,NULL);
