...
void __of1x_dump_instructions(of1x_instruction_group_t group, bool sbo){
	...
	if( has_apply_actions ){
		...
		__of1x_dump_action_group(group.instructions[OF1X_SAFE_IT_TYPE_INDEX(OF1X_IT_APPLY_ACTIONS)].apply_actions, sbo);
	}
	if( has_write_actions ){
		ROFL_PIPELINE_INFO_NO_PREFIX("\n\t\t\tWR.ACTIONs:");
		__of1x_dump_write_actions(group.instructions[OF1X_SAFE_IT_TYPE_INDEX(OF1X_IT_WRITE_ACTIONS)].write_actions, sbo);
	}	
}
