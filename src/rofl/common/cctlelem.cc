...
uint32_t
cctlelem::n_port_get_free_portno()
...
void
cctlelem::up_port_attach(
		uint32_t portno,
		cadapter *adaptor,
		cofport *ofport) throw (eCtlElemExists)
{
	WRITELOG(CFWD, DBG, "cctlelem(%p)::up_port_attach() %s", this, c_str());
...
void
cctlelem::up_port_detach(
		uint32_t portno,
		cadapter *adapter,
		cofport *ofport) throw (eCtlElemNotFound)
{
	WRITELOG(CFWD, DBG, "cctlelem(%p)::up_port_detach() %s", this, c_str());
...
void
cctlelem::up_port_modify(
			uint32_t portno,
			cadapter *adapter,
			cofport *ofport) throw (eCtlElemNotFound)
{
	WRITELOG(CFWD, DBG, "cctlelem(%p)::up_port_modify() %s", this, c_str());
...
void
cctlelem::up_experimenter_message(
		cadapter *ctlmod,
		...
		const cofmatch & m)
{
	try {
		...
		cofmatch match(m);
		WRITELOG(CFWD, DBG, "cctlelem(%s)::down_fsp_open() rcvd cofmatch from cctlmod(%p):\n%s",
		...
		const cofmatch & m)
{
	try {
		...
		cofmatch match(m);
		WRITELOG(CFWD, DBG, "cctlelem(%s)::down_fsp_close() rcvd cofmatch from cctlmod(%p):\n%s",
				dpname.c_str(), ctlmod, match.c_str());
...
void
cctlelem::handle_packet_out(
		cofctrl *ofctrl,
		cofpacket *pack)
{
	WRITELOG(CFWD, DBG, "cctlelem(%s)::handle_packet_out() "
						  "pack: %s\n"
...
void
cctlelem::handle_packet_in(
		cofdpath *sw,
		cofpacket *pack)
{
	WRITELOG(CFWD, DBG, "cctlelem(%s)::handle_packet_in() pack:%s",
	...
	try {
		...
		WRITELOG(CFWD, DBG, "cctlelem(%s)::handle_packet_in() "
...
void
cctlelem::handle_dpath_close(cofdpath *sw)
{
...
void
cctlelem::handle_ctrl_open(cofctrl *ctrl)
{
...
void
cctlelem::handle_ctrl_close(cofctrl *ctrl)
{
...
void
cctlelem::handle_port_status(
		cofdpath *sw,
		cofpacket *pack,
		cofport *port)
{
	WRITELOG(CFWD, DBG, "cctlelem(%s)::handle_port_status() %s", s_dpid.c_str(), cfwdelem::c_str());
...
void
cctlelem::handle_flow_removed(
		cofdpath *sw,
		cofpacket *pack)
{
	WRITELOG(CFWD, DBG, "cctlelem(%s)::handle_flow_removed() %s", s_dpid.c_str(), cfwdelem::c_str());
...
void
cctlelem::handle_experimenter_message(
		cofctrl *ofctrl,
		cofpacket *pack)
{
...
cctlelem::handle_experimenter_message(
		...
		cofpacket *pack)
{
	WRITELOG(CFWD, DBG, "cctlelem(%s)::handle_experimenter_message() pack: %s",
			dpname.c_str(),
			pack->c_str());
		...
		cftentry *fte)
{
	...
	try {
		...
		WRITELOG(CFWD, DBG, "cctlelem(%s)::handle_flow_mod()\n"
				"old_match: %s\n"
		...
		cgtentry *gte)
{
	...
	try {
		...
		WRITELOG(CFWD, DBG, "cctlelem(%s)::handle_group_mod()", dpname.c_str());

		dpid = filter_buckets(pack->buckets, ge.buckets);

		WRITELOG(CFWD, DBG, "cctlelem(%s)::handle_group_mod() "
				"old-buckets: %s\n"
				"new-buckets: %s\n",
				dpname.c_str(),
				pack->buckets.c_str(),
...
cadapter*
cctlelem::find_adaptor_by_portno(
		uint32_t portno) throw (eCtlElemNotFound)
{
	if (n_ports.find(portno) == n_ports.end())
	{
		throw eCtlElemNotFound();
	}
...
uint64_t
cctlelem::filter_actions(
		cofaclist &old_actions,
		cofaclist &new_actions,
		cofmatch &match,
		cpacket *pack) throw (eCtlElemInval, eNotImplemented)
{
	uint64_t dpid = 0;

	WRITELOG(CFWD, DBG, "cctlelem(%s)::filter_actions()\n"
					"old-match: %s\nold-actions: %s",
					dpname.c_str(),
					match.c_str(),
					old_actions.c_str());
