...
class eCtlElemBase : public eFwdElemBase {};
class eCtlElemInval : public eCtlElemBase {};
class eCtlElemExists : public eCtlElemBase {};
class eCtlElemNotFound : public eCtlElemBase {};
class eCtlElemDetached : public eCtlElemBase {};

class eCtlElemEndpntNotFound : public eCtlElemBase {};
...
#if 0
protected: // methods called by classes derived from cctlelem for sending FLOW-MOD and PACKET-OUT to layer (N-1)
...
public: // methods called by cadaptor instances registered within cctlelem
		// these must be overwritten by classes derived from cctlelem in order to
...
protected:
	...
	/* methods from cfwdelem overwritten by cctlelem
	 * all remaining methods are overwritten in derived controller classes
	 * cethctl, cipctl, etc.
	 *
	 * only "up" messages are handled in cctlelem
	 */
...
private: // methods
	...
	filter_actions(
			...
			cofmatch& match,
			cpacket *pack = NULL) throw (eCtlElemInval, eNotImplemented);
...
private: // data structures
		...
		cctlelem *pctlelem;	// adapter container hosting this cadapter
...
public:
	...
	cadapter(
			cctlelem *__ctlelem,
			uint64_t __dpid = 0) throw (eCtlElemInval) :
		pctlelem(__ctlelem),
		dpid(__dpid)
	{
		if (NULL == pctlelem)
	...
	cctlelem&
	ctlelem() throw (eCtlElemDetached)
	{
		if (NULL == pctlelem)
			throw eCtlElemDetached();
		return (*pctlelem);
	};
...
public: // lightweight OpenFlow interface for access by cctlelem
