...
public:
	...
	std::ostream& str_sl(std::ostream& os) const {
		os << indent(0) << "<cofintructions ofp-version:" << (int)ofp_version << " >" << std::endl;
		for (std::map<uint16_t, cofinstruction*>::const_iterator
				it = instmap.begin(); it != instmap.end(); ++it) {
			indent i(2);
			switch (it->second->get_type()) {
			case openflow::OFPIT_GOTO_TABLE:
				get_inst_goto_table().str_sl(os); break;
			case openflow::OFPIT_WRITE_METADATA:
				get_inst_write_metadata().str_sl(os); break;
			case openflow::OFPIT_WRITE_ACTIONS:
				get_inst_write_actions().str_sl(os); break;
			case openflow::OFPIT_APPLY_ACTIONS:
				get_inst_apply_actions().str_sl(os); break;
			case openflow::OFPIT_CLEAR_ACTIONS:
				get_inst_clear_actions().str_sl(os); break;
			case openflow::OFPIT_METER:
				get_inst_meter().str_sl(os); break;
			case openflow::OFPIT_EXPERIMENTER:
				get_inst_experimenter().str_sl(os); break;
			default:
