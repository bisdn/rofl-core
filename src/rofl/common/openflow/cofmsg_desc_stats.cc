...
cofmsg_desc_stats::cofmsg_desc_stats(
		uint8_t of_version,
		uint32_t xid,
		uint16_t flags,
		cofdesc_stats_reply const& desc_stats) :
	cofmsg_stats(of_version, OFPT_STATS_REPLY, xid, OFPST_DESC, flags),
...
void
cofmsg_desc_stats::resize(size_t len)
{
	cofmsg::resize(len);
	switch (get_version()) {
	case OFP10_VERSION: {
...
size_t
cofmsg_desc_stats::length() const
{
	switch (get_version()) {
	case OFP10_VERSION: {
...
void
cofmsg_desc_stats::pack(uint8_t *buf, size_t buflen)
{
...
void
cofmsg_desc_stats::unpack(uint8_t *buf, size_t buflen)
{
...
cofdesc_stats_reply&
cofmsg_desc_stats::get_desc_stats()
{
	return desc_stats;
}
