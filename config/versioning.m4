...
then
	...
	if test -d $srcdir/.git ; then
		...
		AC_DEFINE_UNQUOTED([ROFL_BUILD],["$_ROFL_GIT_BUILD"])
		AC_DEFINE_UNQUOTED([ROFL_BRANCH],["$_ROFL_GIT_BRANCH"])
		AC_DEFINE_UNQUOTED([ROFL_DESCRIBE],["$_ROFL_GIT_DESCRIBE"])
...
AC_DEFINE_UNQUOTED([ROFL_VERSION_],["$ROFL_VERSION"])
